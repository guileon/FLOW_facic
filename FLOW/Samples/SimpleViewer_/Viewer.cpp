...
void SampleViewer::display()
{
	...
	{
		openni::RGB888Pixel* ptScreen = screen;
		openni::DepthPixel* pDepthRow = (openni::DepthPixel*)m_depthFrame.getData();
		...
		for (int y = 0; y < m_depthFrame.getHeight(); ++y)
		{
			...
			for (int x = 0; x < m_depthFrame.getWidth(); ++x, ++pDepth, ++pTex, ++ptScreen)
			{
				if (*pDepth!= 0)
				{
						nHistValue = m_pDepthHist[*pDepth];
						if(*pDepth < min)
							min = *pDepth;
						if(*pDepth > max)
							max = *pDepth;
					/*
					pTex->r = nHistValue*2.5;
					pTex->g = nHistValue*2.5;
					pTex->b = nHistValue*2.5;
					*/
					pTex->r = nHistValue*2.8;
					pTex->g = nHistValue*2.8;
					pTex->b = nHistValue*2.8;
					*ptScreen = *pTex;
